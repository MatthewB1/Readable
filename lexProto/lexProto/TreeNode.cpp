...
template<typename T>
void TreeNode<T>::setLeft(const TreeNode<T>* const left){
	this->left = left;
}
...
template<typename T>
int TreeNode<T>::getTreeDepth()
{
	//if already at head, return depth
	if (parent == nullptr) return getDepthFromNode();
...
int TreeNode<T>::getDepthOfNode()
{
	...
	if (parent == nullptr) return 0;
		...
		//when there is no parent, we are at the head, return count for depth of original node
		return count;
	}
}

template<typename T>
int TreeNode<T>::getDepthFromNode()
{
